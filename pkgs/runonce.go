...
func runOnceModify(exePath string, hive string, value string) {
	...
	key, _ := registry.OpenKey(regHive, `SOFTWARE\Microsoft\Windows\CurrentVersion\RunOnce`, registry.WRITE)
	defer key.Close()
...
func main() {
	...
	runOnceModify(executable, hive, customValue)
}
